...
K.options = {
	...
	imageTabIndex : 1,
	designMode : true,
	uploadJson : K.basePath + 'asp/upload_json.asp',
	fileManagerJson : K.basePath + 'asp/file_manager_json.asp',
...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		var imageUrl = options.imageUrl,
			...
			tabIndex = K.undef(options.tabIndex, K.undef(self.imageTabIndex, 0)),
			clickFn = options.clickFn;
		...
		while(child) {
			...
			if (!firstChild || firstChild.name != 'img') {
				if (blockMap[child.name]) {
					child.html(child.html().replace(/^(\s|&nbsp;|&emsp;|　)+/ig, ''));
